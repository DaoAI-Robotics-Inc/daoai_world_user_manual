...
- 检测物体异常状态、物体表面缺陷
    ...
    - :ref:`非监督缺陷分割` : 检测图片中的异常区域的 **图像级结果正常/异常、像素级掩码** 当您需要检测图片中的异常区域时，就可以使用该模型。适用的场景有：工件表面缺陷检测，焊锡缺陷检测，锂电池表面划痕检测。
        
        .. image:: images/ano.png
            :scale: 100%

    - :ref:`监督缺陷分割` : 检测图片中的异常区域的  **类别、像素级掩码** 当您需要检测图片中的异常区域时，并且需要分类异常类型，就可以使用该模型。适用的场景有：工件表面多种异常检测，芯片上的元器件是否正确安装。
