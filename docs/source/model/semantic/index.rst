...
**语义分割** 会学习并判断物体是否处在异常状态，并使用多边形标注出物体的异常区域以及异常类别。
...
.. note::
    ...
    1. 语义分割 项目在训练时，需要确保分配的训练集中所包含的图片数量小于等于该数据集中全部无损图片的数量，否则可能会导致训练失败。 同时，数据集中所包含的损坏图片的数量过少也会导致训练结果不理想。
    
    2. 语义分割 于其他项目不同，默认不添加任何数据增强选项。