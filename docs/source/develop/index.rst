...
想要了解最新版本的功能？ 请参考 :ref:`SDK版本更新培训视频`
...
2. :ref:`本地HTTP推理`
    ...
    `本地HTTP推理 <./inference_service.html>`_  分别支持 C++ 和 C#.
    
    本地HTTP推理是基于本地 SDK 环境中运行的一种特殊实现，利用 HTTP API 与 DWSDK 进行交互完成推理。
...
4. :ref:`DaoAI 非监督缺陷检测`
    ...
    DaoAI World SDK 内置了完整的非监督缺陷检测能力，与平台上的模型一致之处在于都提供了强大的推理功能；不同之处在于，
    SDK 额外支持本地自主训练，让您在无需连接 DaoAI World 服务器的情况下，管理数据并训练专属模型。详情请参考 :ref:`DaoAI 非监督缺陷检测`
    ...
    - **自主训练模型**：
        - 利用用户提供的图像数据（好的样本为主），训练一个专属的缺陷检测模型。

    优点：
        - **超低样本需求**：仅需 一张正常样本，即可构建检测模型并完美识别各种缺陷，极大降低数据收集和标注成本。
        - **高适配性**：支持多种数据格式和硬件环境。
        - **强大的异常检测能力**：通过非监督学习方法，能够高效捕捉异常模式。
        - **易于集成**：与本地项目无缝结合，提供灵活的接口支持。

    使用场景：
        - 需要对特定产品进行缺陷检测的工业生产线。
        - 需要灵活调节模型参数以适应多样化数据的研发项目。
        - 对隐私性要求较高的场景，无需将数据上传至云端。

通过以上几种 SDK 和服务的组合，开发者可以根据需求选择最适合的工具和方式来实现高效、灵活的推理应用开发。
